...
export default function (revealElement, options) {
  ...
  function getProgress() {
    ...
    if (currentSlide) {
      ...
      if (allFragments.length > 0) {
        let visibleFragments = currentSlide.querySelectorAll(
          '.fragment.visible',
        );
  ...
  function navigateLeft() {
    ...
    if (config.rtl) {
      if (
        (overview.isActive() || fragments.next() === false) &&
    ...
    // Normal navigation
      ...
      (overview.isActive() || fragments.prev() === false) &&
      availableRoutes().left
    ) {
  ...
  function navigateNext() {
    ...
    if (fragments.next() === false) {
      ...
      if (
        ...
        routes.right &&
        config.loop &&
        isLastVerticalSlide(currentSlide)
      ) {
        routes.down = false;
      }
